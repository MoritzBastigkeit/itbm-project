...
```{r variable plots, fig.height=2.5, fig.align='center'}
...
Following the WoE transformation of the datasets, a logistic regression model was employed to model the log-odds of an invidiual defaulting as a linear function of the selected predictor variables. The WoE transformed  outlined in equation 2 in the chapter Methodology, 
...
The selected predictors were determined earlier using the **Information Value (IV)** table already computed as:

```{r, echo=FALSE}
kable(iv.table, caption = "Table: Information Value of Selected Variables")
```
...
```{r, echo=FALSE}
kable(bins.list$duration.in.month, 
      caption = "Table: Duration-in-Month Bin Structure (Counts, PD, WoE, Points)")
```
...
```{r, echo=FALSE}
kable(summary(data_woe.glm)$coefficients,
      caption = "Table: WoE-based Logistic Regression Coefficients (Est., StdErr, z, p)")
```
...
```{r, echo=FALSE}
kable(formatSummary(summary(data_f.glm)),
      caption = "Table: Original-variable GLM (Selected Predictors)")
```
...
```{r, echo=FALSE}
kable(ProbPredAccuracy$binomial_metric,
      caption = "Table: Probability-Prediction Metrics (RMSE, AUC, Gini)")
```

And for scorecard points:

```{r, echo=FALSE}
kable(ScorePredAccuracy$binomial_metric,
      caption = "Table: Score-Prediction Metrics (RMSE, AUC, Gini)")
```
...
```{r, echo=FALSE}
kable(psi.list$psi, caption = "Table: Population Stability Index (PSI)")
```
