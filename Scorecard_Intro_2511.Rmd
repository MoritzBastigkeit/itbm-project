...
bins.list$duration.in.month %>% 
  select(variable, bin, count, count_distr, posprob, woe, bin_iv, total_iv) %>% 
  kbl(
...
## Scorecard Validation — Gini IS & OoS
...
p1 <- get_plot(bins.list$status.of.existing.checking.account) 
p2 <- get_plot(bins.list$duration.in.month)
p3 <- get_plot(bins.list$credit.history)
p4 <- get_plot(bins.list$savings.account.and.bonds)
p5 <- get_plot(bins.list$purpose)
